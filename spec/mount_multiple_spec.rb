...
describe SalebotUploader::Mount do
  let(:klass) { Class.new.send(:extend, SalebotUploader::Mount) }
  let(:uploader) { Class.new(SalebotUploader::Uploader::Base) }
  ...
  describe '.mount_uploaders' do
    ...
    describe 'behavior of subclassed uploaders' do
      let(:uploader_1) do
        Class.new(SalebotUploader::Uploader::Base) do
      ...
      describe 'if the images fails to be processed' do
        before do
          uploader.class_eval do
            process :monkey
            def monkey
              raise SalebotUploader::ProcessingError, 'Ohh noez!' if file.path =~ /test\.jpg/
            end
          end
        end
    ...
    describe '#images_cache=' do
      before do
        allow(instance).to receive(:write_uploader)
        allow(instance).to receive(:read_uploader).and_return(nil)
        SalebotUploader::SanitizedFile.new(test_file_stub).copy_to(public_path('uploads/tmp/1369894322-123-0123-1234/test.jpg'))
      end
    ...
    describe '#images_integrity_errors' do
      ...
      describe 'when an integrity check fails' do
        ...
        context 'when file is cached' do
          ...
          it { is_expected.to include(a_kind_of(SalebotUploader::IntegrityError)) }
      ...
      describe 'when an processing error occurs' do
        before do
          uploader.class_eval do
            process :monkey
            def monkey
              raise SalebotUploader::ProcessingError, 'Ohh noez!'
            end
          end
        end
        ...
        context 'when file is cached' do
          ...
          it { is_expected.to include(a_kind_of(SalebotUploader::ProcessingError)) }
        end
    ...
    describe '#images_download_errors' do
      ...
      context "when file couldn't be found" do
        ...
        it { is_expected.to include(a_kind_of(SalebotUploader::DownloadError)) }
      end
    end
  ...
  describe '#mount_uploaders without an uploader' do
    let(:klass) do
      Class.new.tap do |k|
        k.send(:extend, SalebotUploader::Mount)
    ...
    describe '#images' do
      ...
      it 'returns an instance of a subclass of SalebotUploader::Uploader::Base' do
        expect(instance.images[0]).to be_a(SalebotUploader::Uploader::Base)
      end
  ...
  describe '#mount_uploaders with a block' do
    describe 'and no uploader given' do
      ...
      it 'returns an instance of a subclass of SalebotUploader::Uploader::Base' do
        is_expected.to be_a(SalebotUploader::Uploader::Base)
      end
    ...
    describe 'and an uploader given' do
      let!(:uploader) do
        Class.new(SalebotUploader::Uploader::Base).tap do |u|
          u.version :thumb do
            version :mini
            version :maxi
          end
      ...
      it 'should give the generated class a stable name' do
        expect(first_image.class).to eq klass.const_get :SalebotUploaderImagesUploader
      end
    end
  end
  ...
  describe '#mount_uploaders with :ignore_integrity_errors => false' do
    ...
    context 'when a cached image fails an integrity check' do
      it { expect { instance.images = [test_file_stub] }.to raise_error(SalebotUploader::IntegrityError) }
    end
    ...
    context 'when a downloaded image fails an integrity check' do
      ...
      it { expect {instance.remote_images_urls = ["http://www.example.com/#{test_file_name}"]}.to raise_error(SalebotUploader::IntegrityError) }
    end
  end
  ...
  describe '#mount_uploaders with :ignore_processing_errors => false' do
    ...
    context 'when a cached image fails an integrity check' do
      it { expect { instance.images = [test_file_stub] }.to raise_error(SalebotUploader::ProcessingError) }
    end
    ...
    context 'when a downloaded image fails an integrity check' do
      ...
      it { expect {instance.remote_images_urls = ["http://www.example.com/#{test_file_name}"]}.to raise_error(SalebotUploader::ProcessingError) }
    end
  end
    ...
    before do
      uploader.class_eval do
        def download!(uri, headers = {})
          raise SalebotUploader::DownloadError
        end
      end
    end

    context 'when the image fail to be processed' do
      it { expect {instance.remote_images_urls = ["http://www.example.com/#{test_file_name}"]}.to raise_error(SalebotUploader::DownloadError) }
    end
  end
