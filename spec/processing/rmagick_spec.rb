...
describe SalebotUploader::RMagick, :rmagick => true do

  let(:klass) { Class.new(SalebotUploader::Uploader::Base) { include SalebotUploader::RMagick } }
  ...
  before do
    ...
    allow(instance).to receive(:file).and_return(SalebotUploader::SanitizedFile.new(landscape_file_copy_path))
  end
  ...
  describe "#rmagick_image" do
    ...
    context "with a remotely stored file" do
      class RemoteFile < SalebotUploader::SanitizedFile # rubocop:disable Lint/ConstantDefinitionInBlock
  ...
  describe "test errors" do
    context "invalid image file" do
      ...
      it "fails to process a non image file" do
        expect {instance.resize_to_limit(200, 200)}.to raise_exception(SalebotUploader::ProcessingError, /^Failed to manipulate, maybe it is not an image\?/)
      end
      ...
      it "uses I18n" do
        change_locale_and_store_translations(:nl, :errors => {
          ...
          expect {instance.resize_to_limit(200, 200)}.to raise_exception(SalebotUploader::ProcessingError, /^Kon bestand niet bewerken, misschien is het geen beeld bestand\?/)
        end
      end

      it "doesn't suppress errors when translation is unavailable" do
        change_locale_and_store_translations(:foo, {}) do
          expect { instance.resize_to_limit(200, 200) }.to raise_exception( SalebotUploader::ProcessingError )
        end
      end
  ...
  describe "when working with frames" do
    before do
      ...
      klass.send :include, SalebotUploader::RMagick
    end
