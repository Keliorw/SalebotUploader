...
describe SalebotUploader::ActiveRecord do
  ...
  describe '#mount_uploader' do
    ...
    describe '#image' do
      ...
      it 'resets cached value on record reload' do
        @event.image = SalebotUploader::SanitizedFile.new(stub_file('new.jpeg', 'image/jpeg'))
        @event.save!
      ...
      context 'with SalebotUploader::MiniMagick' do
        before(:each) do
          @uploader.send(:include, SalebotUploader::MiniMagick)
        end
      ...
      context 'with SalebotUploader::RMagick', :rmagick => true do
        before(:each) do
          @uploader.send(:include, SalebotUploader::RMagick)
        end
      ...
      context 'when validating processing' do
        before do
          @uploader.class_eval do
            process :monkey
            def monkey
              raise SalebotUploader::ProcessingError
            end
          end
          @event.image = stub_file('test.jpg')
        end
        ...
        it 'should use I18n for processing errors without messages' do
          ...
          change_locale_and_store_translations(:pt, :activerecord => {
            :errors => {
              :messages => {
                :salebot_uploader_processing_error => 'falha ao processar imagem.'
              }
            }
          }) do
            expect(@event).to_not be_valid
      ...
      context 'when validating download' do
        ...
        it 'should use I18n for download errors without messages' do
          ...
          change_locale_and_store_translations(:pt, :activerecord => {
            :errors => {
              :messages => {
                :salebot_uploader_download_error => 'não pode ser descarregado'
              }
            }
          }) do
            expect(@event).to_not be_valid
  ...
  describe '#mount_uploader removing old files' do
    ...
    describe 'normally' do
      ...
      it 'should only delete the file once when the file is removed' do
        @event.remove_image = true
        expect_any_instance_of(SalebotUploader::SanitizedFile).to receive(:delete).exactly(1).times
        expect(@event.save).to be_truthy
      end
    end
  ...
  describe '#mount_uploader removing old files with versions' do
    ...
    it 'should raise error at save if storage cannot be done, preserving old' do
      allow(@event).to receive(:store_image!).and_raise(SalebotUploader::UploadError)
      Event.transaction do
        @event.image = stub_file('new.jpeg')
        expect{ @event.save }.to raise_error(SalebotUploader::UploadError)
        raise ActiveRecord::Rollback
      end
      expect(File.exist?(public_path('uploads/old.jpeg'))).to be_truthy
    end
      ...
      context 'when validating processing' do
        ...
        it 'should use I18n for processing errors without messages' do
          ...
          change_locale_and_store_translations(:pt, :activerecord => {
            :errors => {
              :messages => {
                :salebot_uploader_processing_error => 'falha ao processar imagesm.'
              }
            }
          }) do
            expect(@event).to_not be_valid
