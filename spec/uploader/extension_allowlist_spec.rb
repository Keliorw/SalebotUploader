...
describe SalebotUploader::Uploader do
  before do
    @uploader_class = Class.new(SalebotUploader::Uploader::Base)
    ...
    context "when there is an allowlist" do
      context "when the allowlist is an array of values" do
        ...
        it "raises an integrity error if the file has not an allowlisted extension" do
          ...
          expect {
            @uploader.cache!(File.open(file_path('test.jpg')))
          }.to raise_error(SalebotUploader::IntegrityError, 'You are not allowed to upload "jpg" files, allowed types: txt, doc, xls')
        end
      ...
      context "when the allowlist is a single value" do
        it "accepts a single extension string value" do
          ...
          expect { @uploader.cache!(File.open(file_path('test.jpg'))) }.to raise_error(SalebotUploader::IntegrityError)
        end

        it "accepts a single extension regular expression value" do
          allow(@uploader).to receive(:extension_allowlist).and_return(/jpe?g/)

          expect { @uploader.cache!(File.open(file_path('bork.txt')))}.to raise_error(SalebotUploader::IntegrityError)
    ...
    context "when there is a whitelist" do
      ...
      it "looks for extension_allowlist first for I18n translation" do
        ...
        change_locale_and_store_translations(:nl, :errors => {
          ...
          expect {
            @uploader.cache!(File.open(file_path('test.jpg')))
          }.to raise_error(SalebotUploader::IntegrityError, 'Het is niet toegestaan om "jpg" bestanden te uploaden; toegestane bestandstypes: txt, doc, xls')
        end
      end
    end
  end
