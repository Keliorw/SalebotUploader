    ...
    context "when there is a denylist" do
      describe "deprecation" do
        ...
        it "shows up" do
          ...
          expect { uploader.cache!(ruby_file) }.to raise_error(SalebotUploader::IntegrityError)
        end
      end
      ...
      context "when the denylist is an array of values" do
        ...
        it "raises an integrity error if the file has a denylisted content type" do
          ...
          expect { uploader.cache!(ruby_file) }.to raise_error(SalebotUploader::IntegrityError, 'You are not allowed to upload image/png files')
        end
    ...
    context "when there is a blacklist" do
      ...
      it "looks for content_type_denylist first for I18n translation" do
        ...
        change_locale_and_store_translations(:nl, :errors => {
          ...
          expect { uploader.cache!(ruby_file) }.to raise_error(SalebotUploader::IntegrityError, 'Het is niet toegestaan om image/png bestanden te uploaden')
        end
      end
    end
  end
