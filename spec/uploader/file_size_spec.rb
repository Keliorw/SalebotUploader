    ...
    describe "file size range" do
      ...
      context "when below the minimum" do
        ...
        it "raises an integrity error" do
          expect { subject }.to raise_error(SalebotUploader::IntegrityError, 'File size should be greater than 2 MB')
        end
      end
      ...
      context "when above the maximum" do
        ...
        it "raises an integrity error" do
          expect { subject }.to raise_error(SalebotUploader::IntegrityError, 'File size should be less than 10 Bytes')
        end
      end
