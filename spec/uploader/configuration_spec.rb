...
describe SalebotUploader do
  describe '.configure' do
    before do
      SalebotUploader::Uploader::Base.add_config :test_config
      SalebotUploader.configure { |config| config.test_config = 'foo' }
    end
    after do
      SalebotUploader::Uploader::Base.singleton_class.send :undef_method, :test_config
      SalebotUploader::Uploader::Base.singleton_class.send :undef_method, :test_config=
      SalebotUploader::Uploader::Base.send :undef_method, :test_config
      SalebotUploader::Uploader::Base.send :undef_method, :test_config=
    end

    it 'proxies to Uploader configuration' do
      expect(SalebotUploader::Uploader::Base.test_config).to eq('foo')
    end
  end
end

describe SalebotUploader::Uploader::Base do
  ...
  describe '.storage' do
    ...
    it 'defaults to file' do
      expect(uploader_class.storage).to eq(SalebotUploader::Storage::File)
    end
    ...
    context 'when inherited' do
      ...
      it 'remembers the storage' do
        expect(subclass.storage).to eq(SalebotUploader::Storage::Fog)
      end
      ...
      it "'s changeable" do
        ...
        expect(subclass.storage).to eq(SalebotUploader::Storage::File)
      end
    end

    it 'raises UnknownStorageError when set unknown storage' do
      expect{ uploader_class.storage :unknown }.to raise_error(SalebotUploader::UnknownStorageError, 'Unknown storage: unknown')
    end
  end
  ...
  describe '.cache_storage' do
    it 'returns the same storage as given by #storage' do
      uploader_class.storage :file
      expect(uploader_class.new.send(:cache_storage)).to be_a(SalebotUploader::Storage::File)
      uploader_class.storage :fog
      expect(uploader_class.new.send(:cache_storage)).to be_a(SalebotUploader::Storage::Fog)
    end
