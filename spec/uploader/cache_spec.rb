...
describe SalebotUploader::Uploader do
  ...
  context "permissions" do
    ...
    describe "with ensuring multipart form deactivated" do
      before do
        SalebotUploader.configure { |config| config.ensure_multipart_form = false }
      end
  ...
  describe '#cache!' do
    before { allow(SalebotUploader).to receive(:generate_cache_id).and_return(cache_id) }

    context "when ensure_multipart_form is true" do
      before { SalebotUploader.configure { |config| config.ensure_multipart_form = true } }

      it "raises an error when trying to cache a string" do
        expect { uploader.cache!(test_file_path) }.to raise_error(SalebotUploader::FormNotMultipart)
      end

      it "raises an error when trying to cache a pathname" do
        expect { uploader.cache!(Pathname.new(test_file)) }.to raise_error(SalebotUploader::FormNotMultipart)
      end
    end
    ...
    context "when a file is cached" do
      ...
      it "caches the file" do
        expect(uploader.file).to be_an_instance_of(SalebotUploader::SanitizedFile)
      end
      ...
      context 'negative cache id' do
        ...
        before do
          allow(SalebotUploader).to receive(:generate_cache_id).and_return(cache_id)
        end
    ...
    describe "with the move_to_cache option" do
      ...
      before do
        ...
        allow(SalebotUploader).to receive(:generate_cache_id).and_return(cached_id)
      ...
      context "set to true" do
        ...
        describe "method usage" do
          ...
          it "uses #move_to during #cache!" do
            ...
            expect_any_instance_of(SalebotUploader::SanitizedFile).to receive(:move_to).with(workfile_path, permission, permission).and_return(moved_file)
            expect(moved_file).to receive(:move_to).with(cached_path, permission, permission, true)
          end

          it "doesn't use #copy_to during #cache!" do
            expect_any_instance_of(SalebotUploader::SanitizedFile).not_to receive(:copy_to)
          end
        end
      end
      ...
      context "set to false" do
        ...
        describe "method usage" do
          ...
          it "uses #move_to during cache!" do
            ...
            expect_any_instance_of(SalebotUploader::SanitizedFile).to receive(:copy_to).with(workfile_path, permission, permission).and_return(moved_file)
            expect(moved_file).to receive(:move_to).with(cached_path, permission, permission, true)
          end

          it "doesn't use #move_to during #cache!" do
            expect_any_instance_of(SalebotUploader::SanitizedFile).not_to receive(:move_to).with(workfile_path, permission, permission)
          end
        end
      end
    end
  ...
  describe '#retrieve_from_cache!' do
    ...
    it "raises an error when the cache_id has an invalid format" do
      ...
      }.to raise_error(SalebotUploader::InvalidParameter)
    end
  ...
  describe '.generate_cache_id' do
    it 'generates dir name based on UTC time' do
      Timecop.freeze(Time.at(1369896000)) do
        expect(SalebotUploader.generate_cache_id).to match(/\A1369896000-\d+-\d+-\d+\Z/)
      end
    end

    it 'generates dir name with a counter substring' do
      counter = SalebotUploader.generate_cache_id.split('-')[2].to_i

      expect(SalebotUploader.generate_cache_id.split('-')[2].to_i).to eq(counter + 1)
    end

    it 'generates dir name using constant length for the counter' do
      allow(SalebotUploader::CacheCounter).to receive(:increment).and_return(1234567890)

      expect(SalebotUploader.generate_cache_id.split('-')[2].length).to eq(4)
    end
  end
