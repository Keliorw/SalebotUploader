...
describe SalebotUploader::Downloader::Base do
  ...
  subject { SalebotUploader::Downloader::Base.new(uploader) }
  ...
  context "with unicode symbols in URL" do
    ...
    it "downloads a file" do
      expect(remote_file).to be_an_instance_of(SalebotUploader::Downloader::RemoteFile)
    end
    ...
    let(:authentication_headers) do
      {
        ...
        'User-Agent'=>"SalebotUploader/#{SalebotUploader::VERSION}",
  ...
  context "with missing file" do
    ...
    it "raises an error when trying to download a missing file" do
      expect{ subject.download(uri) }.to raise_error(SalebotUploader::DownloadError)
    end

    it "doesn't obscure original exception message" do
      expect { subject.download(uri) }.to raise_error(SalebotUploader::DownloadError, /could not download file: 404/)
    end
  end
  ...
  context "with server error" do
    ...
    it "doesn't obscure original exception message" do
      expect { subject.download(uri) }.to raise_error(SalebotUploader::DownloadError, /could not download file: 500/)
    end
  end
  ...
  context "with SSRF prevention" do
    ...
    it "prevents accessing local files" do
      expect { subject.download('/etc/passwd') }.to raise_error(SalebotUploader::DownloadError)
      expect { subject.download('file:///etc/passwd') }.to raise_error(SalebotUploader::DownloadError)
    end

    it "prevents accessing internal addresses" do
      expect { uploader.download!("http://169.254.169.254/") }.to raise_error SalebotUploader::DownloadError
      expect { uploader.download!("http://lvh.me/") }.to raise_error SalebotUploader::DownloadError
    end
  end
  ...
  context 'with download_retry_count' do
    ...
    context 'when download_retry_count == 0 ' do
      ...
      it 'throws an exception' do
        expect { subject }.to raise_error SalebotUploader::DownloadError
      end
    end
  ...
  context 'when actually downloading a file' do
    let(:uri) { 'https://raw.githubusercontent.com/Keliorw/SalebotUploader/master/spec/fixtures/test.jpg' }
  ...
  describe '#process_uri' do
    ...
    it "throws an exception on bad uris" do
      uri = '~http:'
      expect { subject.process_uri(uri) }.to raise_error(SalebotUploader::DownloadError)
    end
  end
