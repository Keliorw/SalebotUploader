  ...
  describe "with #{fog_credentials[:provider]} provider", with_retry: !Fog.mocking do
    before do
      ...
      SalebotUploader.configure do |config|
        ...
        config.fog_directory = SALEBOT_UPLOADER_DIRECTORY
      ...
      eval <<~RUBY
        class FogSpec#{fog_credentials[:provider]}Uploader < SalebotUploader::Uploader::Base
          storage :fog
        end
      RUBY
      ...
      @storage = SalebotUploader::Storage::Fog.new(@uploader)
      @directory = @storage.connection.directories.get(SALEBOT_UPLOADER_DIRECTORY) || @storage.connection.directories.create(:key => SALEBOT_UPLOADER_DIRECTORY, :public => true)
    end
    ...
    describe 'with a valid Hash' do
      let(:file) do
        SalebotUploader::SanitizedFile.new(
    ...
    describe 'with a valid Tempfile' do
      let(:file) do
        SalebotUploader::SanitizedFile.new(stub_tempfile('test.jpg', 'image/jpeg'))
      end
    ...
    describe 'with a valid StringIO' do
      let(:file) do
        SalebotUploader::SanitizedFile.new(stub_stringio('test.jpg', 'image/jpeg'))
      end
    ...
    describe 'with a valid File object' do
      let(:file) do
        SalebotUploader::SanitizedFile.new(stub_file('test.jpg', 'image/jpeg'))
      end
    ...
    describe 'with a valid File object with an explicit content type' do
      let(:file) do
        SalebotUploader::SanitizedFile.new(stub_file('test.jpg', 'image/jpeg')).tap do |f|
    ...
    describe 'with a valid path' do
      let(:file) do
        SalebotUploader::SanitizedFile.new(file_path('test.jpg'))
      end
    ...
    describe 'with a valid Pathname' do
      let(:file) do
        SalebotUploader::SanitizedFile.new(Pathname.new(file_path('test.jpg')))
      end

      it_should_behave_like "#{fog_credentials[:provider]} storage accepting files"
    end

    describe 'with a SalebotUploader::Storage::Fog::File' do
      let(:file) do
        SalebotUploader::Storage::Fog::File.new(@uploader, @storage, 'test.jpg').
          tap{|file|
          file.store(SalebotUploader::SanitizedFile.new(
    ...
    describe '#cache_stored_file!' do
      before do
        allow(@uploader).to receive(:store_path).and_return('uploads/test.jpg')
        @fog_file = @storage.store!(SalebotUploader::SanitizedFile.new(stub_file('test.jpg', 'image/jpeg')))
      end
    ...
    describe '#clean_cache!' do
      ...
      before do
        ...
        [five_days_ago, three_days_ago, yesterday, (today - 1.minute)].each do |created_date|
          ...
          Timecop.freeze created_date do
            key = "uploads/tmp/#{SalebotUploader.generate_cache_id}/test.jpg"
          end
      ...
      it 'should be aliased on the SalebotUploader module' do
        Timecop.freeze(today) do
    ...
    describe 'SalebotUploader::Storage::Fog::File' do
      let(:store_path) { 'uploads/test.jpg' }
      let(:fog_public) { true }
      before do
        allow(@uploader).to receive(:store_path).and_return(store_path)
      ...
      describe '#public_url' do
        ...
        when 'AWS'
          ...
          #   expect(@fog_file.public_url).to include("https://#{SALEBOT_UPLOADER_DIRECTORY}.s3-accelerate.amazonaws.com")
          # end
          #
        ...
        context 'with asset_host' do
          ...
          context 'given as a proc' do
            let(:asset_host) { proc { |storage| expect(storage).to be_instance_of ::SalebotUploader::Storage::Fog::File } }
