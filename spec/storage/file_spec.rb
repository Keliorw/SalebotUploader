...
describe SalebotUploader::Storage::File do
  ...
  let(:sanitized_temp_file) { SalebotUploader::SanitizedFile.new(tempfile) }
  ...
  describe '#clean_cache!' do
    ...
    let(:cache_dir) { File.expand_path(uploader_class.cache_dir, SalebotUploader.root) }

    before do
      [five_days_ago, three_days_ago, yesterday, (today - 1.minute)].each do |created_date|
        Timecop.freeze(created_date) do
          FileUtils.mkdir_p File.expand_path(SalebotUploader.generate_cache_id, cache_dir)
        end
      end
    end
    ...
    it "'s aliased on the SalebotUploader module" do
      Timecop.freeze(today) { SalebotUploader.clean_cached_files! }
    ...
    it "clears all files older than now in the new cache directory" do
      uploader_class.root = File.expand_path("new", SalebotUploader.root)
      new_cache_dir = File.expand_path(uploader_class.cache_dir, uploader.root)
      Timecop.freeze(today - 1.minute) { FileUtils.mkdir_p File.expand_path(SalebotUploader.generate_cache_id, new_cache_dir) }
