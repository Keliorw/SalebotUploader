...
describe SalebotUploader::Storage::Fog do
  ...
  describe '.eager_load' do
    after do
      SalebotUploader::Storage::Fog.connection_cache.clear
      SalebotUploader::Uploader::Base.fog_credentials = nil
    end

    it 'caches Fog::Storage instance' do
      SalebotUploader::Uploader::Base.fog_credentials = {
        provider: 'AWS', aws_access_key_id: 'foo', aws_secret_access_key: 'bar'
      }
      expect { SalebotUploader::Storage::Fog.eager_load }.
        to(change { SalebotUploader::Storage::Fog.connection_cache })
    end

    it 'does nothing when fog_credentials is empty' do
      SalebotUploader::Uploader::Base.fog_credentials = {}
      expect { SalebotUploader::Storage::Fog.eager_load }.
        not_to(change { SalebotUploader::Storage::Fog.connection_cache })
    end
  end

  describe SalebotUploader::Storage::Fog::File do
    subject(:file) { SalebotUploader::Storage::Fog::File.new(nil, nil, nil) }
